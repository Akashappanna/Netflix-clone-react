...
const Image = ({ movie, isLarge, fetchUrl, isSmall, pos }) => {
  ...
  return (
    <React.Fragment>
      <div
        className={`relative w-full  md:px-4 px-2 flex justify-center  hover:scale-125 hover:z-50 transition-all duration-300 cursor-pointer
    ${isSmall ? "w-1/4 md:w-1/5 xl:w-1/6 my-3" : ""}`}
      >
        <Link
          to={`/netflix/${movie.id}`}
          state={{ movie: movie, fetchUrl: fetchUrl }}
        >
          <img
            className={`w-32 md:w-44 max-w-fit ${isLarge ? "rounded-3xl" : ""}`}
            src={`${base_url}${
        ...
        {hovered.hover === true && (
          <SmallDesc smallMovieData={smallMovieData} pos={pos} />
        )}
      </div>
    </React.Fragment>
  );
